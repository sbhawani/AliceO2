...
class PID
{
 public:
  ...
  std::unordered_map<std::string_view, std::vector<std::unique_ptr<TH1>>>& getMapOfHisto() { return mapHist2D; }
  const std::unordered_map<std::string_view, std::vector<std::unique_ptr<TH1>>>& getMapOfHisto() const { return mapHist2D; }

 private:
  std::unordered_map<std::string_view, std::vector<std::unique_ptr<TH1>>> mapHist2D;
  ClassDefNV(PID, 1)
};
} // namespace qc
} // namespace tpc
